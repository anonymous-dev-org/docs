---
title: 'Neovim'
description: '"MiniMax" editor configuration built on mini.nvim'
---

The Neovim configuration is called **MiniMax** — a feature-rich setup built almost entirely
on the [mini.nvim](https://github.com/nvim-mini/mini.nvim) library, with select external
plugins for language support, Git, and debugging.
...
|---|---|---|
| `<Leader>a` | AI | `ac` toggle, `af` focus, `ar` resume, `aC` continue, `am` model, `ab` add buffer, `as` send (visual), `aa` accept diff, `ad` deny diff |
...
plugins. Two-stage loading is used for startup performance:
...
| `snacks.nvim` | UI framework (claudecode dependency) |
| `claudecode.nvim` | Claude Code integration |
| `0x0-completion.nvim` | Custom completion plugin |
