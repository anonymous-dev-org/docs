...
<Info>
  ⚠️ **Important**: Firetype extends the Firestore SDK and is not a replacement.
...
<Tabs>
  <Tab title="Server-side">
    ```bash # For server-side applications (Node.js, Cloud Functions, etc.) npm
    install firebase-admin
    ```
  </Tab>
  <Tab title="Client-side">
    ```bash # For client-side applications (browsers, React Native, etc.) npm
    install firebase
    ```
  </Tab>
</Tabs>
...
<Note>
  💡 **Tip**: Firetype works with both SDKs simultaneously if you need to
