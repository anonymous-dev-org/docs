...
| `search_remote` | `search_remote` (0x0 registered tool) | Web search and URL fetch |
| `web` | `WebFetch`, `WebSearch` (provider-delegated) | Provider SDK web tools |
| `bash` | `bash` tool | Shell command execution |
| `task` | `task` tool | Subtask creation / agent delegation |
| `external_directory` | `read`, `search`, `edit`, and other file-based tools | Access to directories outside the project root |
...
Custom (non-native) agents start with all permissions denied. Only tools listed in `actions` (or
the legacy `tools_allowed`) are explicitly allowed, and the agent's `permission` field is layered on top.
...
2. The user sees the request in the TUI and can respond with:
   ...
   - **Allow always** -- adds a permanent `allow` rule and auto-resolves other pending requests
     now covered. The rule is persisted to `agent.<name>.actions.<provider>.<tool>` in config.
   - **Deny always** -- adds a permanent `deny` rule for this permission and pattern, persisted
     to config
...
When a user chooses "Allow always" or "Deny always", the decision is persisted to the config file
under `agent.<agentName>.actions.<providerID>.<toolName>`. This uses the provider SDK's tool names
(e.g., `Bash`, `Edit`, `WebFetch`), not the 0x0 permission names.
