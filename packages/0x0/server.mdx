...
- Set `server.password` in `config.yaml` to require basic auth.
- `server` warns if password is not set.
- `attach` supports `--password` and falls back to `server.password` in config.
...
0x0 attach https://my-host:4096 --password "your-password"
```
